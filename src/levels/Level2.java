...
public class Level2 {
    ...
    public static void main(String[] args) {
        ...
        long sum = teachers.stream().filter(teacher -> teacher.getSubject().equals(Subject.FLUTTER)).mapToInt(teacher -> teacher.getSalary()).sum();/* TO DO 2 */;

        /* TO DO 3: Retourner la moyenne des salaires des enseignants dont le nom commence avec a */
        double average = teachers.stream().filter(teacher -> teacher.getName().startsWith("a")).mapToInt(teacher -> teacher.getSalary()).average().orElse(0);/* TO DO 3 */;
        ...
        Stream<Object> collectors;
        List<Teacher> teachers3 = teachers.stream().filter(teacher -> teacher.getName().startsWith("f")).collect(Collectors.toList())/* TO DO 4 */;
        ...
        /*First way*/
        teachers.stream().filter(teacher -> teacher.getName().startsWith("g")&&teacher.getSubject().equals(Subject.UNITY)).findFirst().ifPresent(System.out::println);/* TO DO 7 */;
        ;
        /*Second way*/
        teachers.stream().filter(teacher -> teacher.getName().startsWith("g")&&teacher.getSubject().equals(Subject.UNITY)).limit(1).forEach(System.out::println);/* TO DO 7 */;

        /* TO DO 7: Afficher le deuxième enseignant dont le nom commence avec s */
        teachers.stream().filter(teacher -> teacher.getName().startsWith("s")).skip(1).limit(1).forEach(System.out::println);/* TO DO 8 */;
