...
public class Level1 {
    ...
    public static void main(String[] args) {
         ...
         */
        teachers.stream().filter(teacher -> teacher.getName().startsWith("n")).filter(teacher -> teacher.getSalary() >10000).forEach(System.out::println);

        /*
         * TO DO 4: Afficher les enseignants JAVA triés par salaire (éliminer les redondances)
         */
        teachers.stream().filter(teacher -> teacher.getSubject().equals(Subject.JAVA)).sorted(((o1, o2) -> o1.getSalary() - o2.getSalary())).distinct().forEach(System.out::println);
         ...
         */
        teachers.stream().filter(teacher -> teacher.getName().startsWith("m")).peek(teacher -> teacher.setSalary(teacher.getSalary()+200)).max((t1 ,t2) -> t1.getSalary()- t2.getSalary()).ifPresent(System.out::println );
